function multi_drone_rrt_simulation
    ...
    for k = 1:3
        return_paths{k} = rrt_star(goal_positions(k, :), start_positions(k, :), step_size, max_iter, goal_tolerance, obstacles, obstacle_speed);
    end

    interpolated_return_paths = cellfun(@(path) interpolate_path(path, 0.1), return_paths, 'UniformOutput', false);
    ...
    for i = 1:max(cellfun(@(p) size(p, 1), interpolated_return_paths))
        ...
        for k = 1:3
            if i <= size(interpolated_return_paths{k}, 1)
                set(drone_plots(k), 'XData', drones(:, 1) + interpolated_return_paths{k}(i, 1), ...
